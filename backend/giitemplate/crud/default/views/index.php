...
if(empty($jsonV)!=true){
    ...
    if($jsonV["type"]=="text"){
        ...
        echo $hideStr."['class'=>'yii\\grid\\DataColumn','value'=>function(\$data){   return \$data->".$jsonV["name"]."[\$data['".$name."']]??'';},'attribute' => '".$name."'],\n";
...
?>
    ...
    return '<a title="删除" href="javascript:;" clickDelete="0" onclick="backend_delete_data(this,\''.$key.'\',\''.$model-><?= $nameAttribute ?>.'\',\''.$url.'\')" class="ml-5 operation-icon operation-del-icon index-table-delete" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>'; },
                ],
    ],
        ],
    ]); ?>
