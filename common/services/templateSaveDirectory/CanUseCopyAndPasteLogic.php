...
class CanUseCopyAndPasteLogic
{
    ...
    public function create($data=[],$scenario='create'){
        ...
        if(!empty($data)){
          $model = new XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX();
    ...
    public function update($data=[],$scenario='update'){
        ...
        if(!empty($data)){
            $id = $data['id']??0;
            $id = intval($id);
            $model = XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX::findOne(['id'=>$id,'is_delete'=>0]);
            if(empty($model)){
                return $common->getParameterErrorMassage(true); //直接获取参数错误返回
            }
            $model->scenario = $scenario;
    ...
    public function delete($data=[],$scenario='delete'){
        ...
        if(!empty($data)){
            $id = $data['id']??0;
            $id = intval($id);
            $model = XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX::findOne(['id'=>$id]);
            if(empty($model)){
                return $common->getParameterErrorMassage(true); //直接获取参数错误返回
            }
            $model->scenario = $scenario;
    ...
    public function detail($data=[],$scenario='list'){
        ...
        if(!empty($data)){
            ...
            $modelModel = new XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX();
    ...
    public function list($data=[],$scenario='list'){
        $common = new BaseCommon();
        //可创建与后台相同的Search对象，也可以继承
        $searchModel = new XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXSearch();
